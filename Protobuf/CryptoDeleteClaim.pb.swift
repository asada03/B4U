...
struct Proto_CryptoDeleteClaimTransactionBody {
  ...
  /// the account that should have a claim deleted
...
extension Proto_CryptoDeleteClaimTransactionBody: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
    func _protobuf_generated_isEqualTo(other: Proto_CryptoDeleteClaimTransactionBody) -> Bool {
        return false
    }
    
  static let protoMessageName: String = _protobuf_package + ".CryptoDeleteClaimTransactionBody"
  static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    2: .same(proto: "accountIDToDeleteFrom"),
    3: .same(proto: "hashToDelete"),
  ]
  ...
  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        switch fieldNumber {
        case 2: try decoder.decodeSingularMessageField(value: &_storage._accountIdtoDeleteFrom)
        case 3: try decoder.decodeSingularBytesField(value: &_storage._hashToDelete)
        default: break
        }
      }
    }
  ...
  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      ...
      if !_storage._hashToDelete.isEmpty {
        try visitor.visitSingularBytesField(value: _storage._hashToDelete, fieldNumber: 3)
      }
    }
